...
from config import NewLaunchConst, GeneralConst, WrongExtensionPopup


class FirstLaunch(wx.Frame):
    def __init__(self, data_file: DataFile, settings: Settings) -> None:
        super().__init__(None, style=wx.CLOSE_BOX, title=NewLaunchConst.TITLE)
    ...
    def _init_ui(self) -> None:
        ...
        self._create_new = wx.Button(panel, label=NewLaunchConst.CREATE_NEW_LABEL)
        self._import_datafile = wx.Button(panel, label=NewLaunchConst.IMPORT_DATAFILE_LABEL)
    ...
    def _on_import_datafile(self, event) -> None:
        ...
        if file_path is not None:
            ...
            if valid:
                self._import_file_to_default_path(file_path)
                self.restart()
